...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        info.append(name)
        info.append(position_id)
        info.append(birthyear)
        info.append(nationality)
        #print(info)
        players.append(info)
    cur.execute("DROP TABLE IF EXISTS Players")
    cur.execute("CREATE TABLE IF NOT EXISTS Players (id INTEGER PRIMARY KEY, name TEXT, position_id INTEGER, birthyear INT, nationality TEXT)")
    #print("excuted and shit")
    for i in range(len(players)):
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear,nationality) VALUES (?,?,?,?,?)", (i, players[i][0], players[i][1], players[i][2], players[i][3]))
...
def nationality_search(countries, cur, conn):
    ...
    cur.execute("SELECT * FROM Players")
    players_fin = []
    player_dict = cur.fetchall()
    for player in player_dict:
        if player[4] in countries:
            players_fin.append((player[1], player[2], player[4]))
...
def birthyear_nationality_search(age, country, cur, conn):
    ...
    thresh = 2023-int(age)
    for player in player_dict:
        if (player[4] == country) and (int(player[3]) < thresh):
            players_fin.append((player[1], player[4], player[3]))
